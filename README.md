...
## Environment
- .NET version: 8.0
- Solution layout: N-layer architecture under `src/` (HackerRank1.Domain, HackerRank1.Application, HackerRank1.Infrastructure, HackerRank1.API) and `tests/`.
...
- run:
```
dotnet run --project src/HackerRank1.API
```
- build:
```
dotnet build HackerRank1.sln
```
- test:
```
dotnet test HackerRank1.sln
```
...
NOTE: Services (LibrariesService and BooksService) and their repositories are registered for Dependency Injection in the API's Startup.cs file.
